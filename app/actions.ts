...
// Content actions
export async function fetchContentByType(type: "course" | "lesson" | "quiz", language?: string) {
  const supabase = await getServerSupabaseClient()
...
export async function searchContent(query: string, language?: string) {
  ...
  let searchQuery = supabase.from("content").select("*").ilike("title", `%${query}%`)
...
// Progress tracking
export async function updateUserProgress(contentId: string, progressPercentage: number, completed: boolean) {
  const supabase = await getServerSupabaseClient()
...
// Download management
export async function markContentAsDownloaded(contentId: string, sizeBytes: number) {
  const supabase = await getServerSupabaseClient()
