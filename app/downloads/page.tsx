...
export default function DownloadsPage() {
  ...
  const totalStorageBytes = downloads?.reduce((total, d) => total + (d.size_bytes || 0), 0) || 0
      ...
      <div className="space-y-6">
        <div className="flex flex-col space-y-2">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold tracking-tight animate-fade-in">Downloaded Content</h1>
            {!isOnline && (
        ...
        <Card className="animate-fade-in" style={{ animationDelay: "0.2s" }}>
          <CardHeader>
            ...
            <CardDescription>
              You have {downloads?.length || 0} items downloaded, using {totalStorageMB} MB of storage
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="progress-bar">
              <div
                ...
                style={{ width: `${Math.min((totalStorageBytes / (100 * 1024 * 1024)) * 100, 100)}%` }}
              ></div>
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {Math.min((totalStorageBytes / (100 * 1024 * 1024)) * 100, 100).toFixed(0)}% of 100 MB used
            </p>
          </CardContent>
        </Card>
                    ...
                    <CardHeader className="py-4">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <CardDescription>
                            {download.content?.type.charAt(0).toUpperCase() + download.content?.type.slice(1)} •
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => removeDownload(download.content_id)}
                            disabled={!isOnline}
                          >
        ...
        <div className="flex justify-center mt-8">
          <Button
            ...
            disabled={!downloads?.length || (!isOnline && downloads?.length > 0)}
          >
            Clear All Downloads
          </Button>
        </div>
